...
try {
  ...
  renameSync(
    ...
    join(projectRoot, "old.package.json"),
  );
  writeFileSync(
    join(projectRoot, "package.json"),
  ...
  console.log({
    projectPackage: join(projectRoot, "package.json"),
    workspacePackage: join(workspaceRoot, "package.json"),
    packageJson: JSON.stringify(projectPackage, null, 2),
  });
} catch (e) {
